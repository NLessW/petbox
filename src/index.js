...
let lastPointApi = { studentId: null, count: 0, result: null };
...
// 포인트 적립 API 호출
async function callPointApi(studentId, count) {
    if (__testMode) {
        return Promise.resolve({ status: 'ok', test: true, mobile: '010-0000-' + studentId, input_cnt: count });
    }
    // [CHG] cyclepet 도메인 + JSON 방식 + unique key + group_cd 포함
    const apiUrl = POINT_API_URL;
    const payload = {
        mobile: '010-000' + String(studentId || ''), // 학번 앞에 010-0000- 붙여서 전화번호 형식으로
...
// [ADD] 회원 확인 API 호출
async function callMemberApi(studentId) {
...
async function finalizeAndReturnHome() {
    ...
    let result = null;
    try {
        if (currentStudentId && depositCount > 0) {
            // [CHG] callPointApi는 unique key + group_cd 포함해 전송
            result = await callPointApi(currentStudentId, depositCount);
            if (result && result.status === 'error') {
                const logLineErr = `[${nowTs()}] RESULT=ERROR device=${
                    deviceConfig.deviceCode || 'UNKNOWN'
                } student_id=${currentStudentId} count=${depositCount} msg=${
                ...
                const logLineOk = `[${nowTs()}] RESULT=OK device=${
                    deviceConfig.deviceCode || 'UNKNOWN'
                } student_id=${currentStudentId} count=${depositCount} raw=${JSON.stringify(result)}`;
        ...
        const logLineEx = `[${nowTs()}] RESULT=EXCEPTION device=${
            deviceConfig.deviceCode || 'UNKNOWN'
        } student_id=${currentStudentId} count=${depositCount} error=${(err && (err.message || err)) || ''}`;
    ...
    if (screenId === 'end-screen') {
        ...
        (async () => {
            ...
            try {
                ...
                let res = null;
                if (sid && cnt > 0) {
                    res = await callPointApi(sid, cnt);
                    lastPointApi = { studentId: sid, count: cnt, result: res };
                    ...
                    const logLineOk = `[${nowTs()}] RESULT=OK device=${
                        deviceConfig.deviceCode || 'UNKNOWN'
                    } student_id=${sid} count=${cnt} raw=${JSON.stringify(res)}`;
                    appendPointLog(logLineOk);
                    ...
                    summary.textContent = '학번 또는 투입 수량이 없어 적립을 진행하지 않았습니다.';
                }
            } catch (err) {
                // 예외 로그
                const logLineEx = `[${nowTs()}] RESULT=EXCEPTION device=${
...
function showErrorScreen(message) {
    ...
    const studentIdEl = document.getElementById('error-phone');
    if (studentIdEl) studentIdEl.textContent = `입력한 학번: ${currentStudentId || '-'}`;
...
function renderProcess(iconKey, message, bgIndex, { spin = false, iconAlt = '' } = {}) {
    try {
        ...
        if (fill) {
            let pct = 0;
            if (iconKey === 'label') pct = 5;
            else if (iconKey === 'openDoor') pct = 25;
...
async function startProcess() {
    ...
    // 문 열림 안내 - 1단계부터 시작
    const openMsg = `문이 열립니다.<br>투입하고 난 뒤.<br>마지막으로 닫기 버튼을 눌러주세요.`;
    ...
    try {
        ...
        if (handleDeviceLost(err)) return;
        console.error('문 열기 명령 전송 실패:', err);
    }
    ...
    return withReadLock(
        () =>
            new Promise((resolve, reject) => {
                ...
                // 타이머 설정/재설정
                const scheduleTimeout = (ms, labelMs = ms) => {
                    try {
                        clearTimeout(timer);
                    } catch {}
                    timer = setTimeout(() => {
                        abortProcessNow(`기기 오류: 모터 오작동(${Math.round(labelMs / 1000)}초 내 완료 신호 없음)`);
...
async function connectToFaduino() {
    try {
        ...
        if (port.readable || port.writable) {
            ...
            // [FIX] 하드락 중이면 유지보수 해제/상태체크 재개 금지
            const isErrorVisible = document.getElementById('error-screen')?.style.display === 'flex';
            const hardLock = !!(typeof window !== 'undefined' && window.__hardLock);
            if (window && !isErrorVisible && !hardLock) {
                window.__maintenanceMode = false;
...
loginSubmit.addEventListener('click', async () => {
    ...
    if (studentId.length !== 5) {
        ...
        showConfirmModal({
            title: '학번 확인',
            lines: ['올바른 학번을 입력하세요.', '(예: 1학년 1반 1번 : 10101)'],
            yesText: '확인',
            noText: '취소',
            onYes: () => {},
            onNo: () => {},
    ...
    try {
        // [NEW] 먼저 회원 확인 API 호출
        const member = await callMemberApi(studentId);

        // 성공 케이스: 회원명 확인 팝업
        if (member?.status === 'success') {
            const uname = member?.data?.user_name || '';
            // API에서 받은 전화번호에서 010-000 제거하여 학번만 표시
            let displayStudentId = studentId;
            if (member?.data?.mobile) {
                // 010-000을 제거하고 나머지만 추출 (예: 010-00012345 -> 12345)
                displayStudentId = String(member.data.mobile).replace(/^010-?000?/, '');
            }
            showConfirmModal({
                title: '회원 확인',
                lines: [`학번 : <b>${displayStudentId}</b>`, `학번 <b>${displayStudentId}</b> 이 맞으신가요?`],
                yesText: '예',
                noText: '아니오',
                onYes: async () => {
                    // 기존 시작 로직 수행
        ...
        // FAIL 이면서 "처음 사용" 메시지인 경우: 학번 재확인 팝업
        const firstUseMsg = 'PETMON에 처음 사용하시는 회원입니다. 학번을 확인하기 위해 한번더 입력 부탁드립니다';
        if (member?.status === 'FAIL' && String(member?.message || '').includes('처음 사용')) {
            showConfirmModal({
                title: '학번 확인',
                lines: [`입력하신 학번: <b>${studentId}</b>`, '이 학번이 맞습니까?'],
                yesText: '예',
                noText: '아니오',
                onYes: async () => {
                    // 신규 회원도 동일하게 시작
...
keypad.addEventListener('click', (e) => {
    ...
    let val = studentIdInput.value.replace(/-/g, '');
    if (key === '←') val = val.slice(0, -1);
    else if (key === 'C') val = '';
    else if (!isNaN(key) && val.length < 5) val += key;
